...
class BertForClassificationDan(BertPreTrainedModel):
    def __init__(self, config, num_labels=2, num_domains=4, is_tok_level_task=False):
        super(BertForClassificationDan, self).__init__(config)
        ...
        self.discriminator = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(in_features=100, out_features=num_domains)
        )
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        reverse_hidden = GradReverseLayerFunction.apply(bert_output)
        adversarial_logits = self.discriminator(reverse_hidden).squeeze()

        return cls_logits, adversarial_logits

    def masked_avg_pooler(self, sequences, masks):
        if masks is None:
        ...
        return torch.sum(masked_sequences.float() / sequence_lengths.float(), dim=1)
...
def calc_loss_and_preds(args, logits, label_ids, num_labels, domain_logits, domain_label_ids, num_domain_labels, n_gpu):
    ...
    loss = task_loss + 0.01*domain_loss
    ...
    # calc domain accuracy
    domain_preds = np.argmax(domain_logits.view(-1, num_domain_labels).detach().cpu().numpy(), axis=1)
    domain_label_ids = domain_label_ids.view(-1).detach().cpu().numpy()
    domain_preds = np.array([pred for idx, pred in enumerate(domain_preds) if domain_label_ids[idx] >= 0])
    domain_label_ids = np.array([label_id for label_id in domain_label_ids if label_id >= 0])

    if n_gpu > 1:
        loss = loss.mean()  # mean() to average on multi-gpu.

    return loss, task_loss, domain_loss, preds, label_ids, domain_preds, domain_label_ids


def evaluate(args, eval_dataloader, model, device, num_labels, num_domain_labels, is_tok_cls=False):
    model.eval()
    eval_loss, eval_task_loss, eval_dom_loss, nb_eval_steps = 0, 0, 0, 0
    accum_preds, accum_label_ids, all_logits = None, None, None
    accum_domain_preds, accum_domain_label_ids, all_domain_logits = None, None, None
    ...
    for eval_element in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            logits, domain_logits = model(input_ids, input_mask)

        loss, task_loss, domain_loss, preds, label_ids, domain_preds, domain_label_ids = \
            calc_loss_and_preds(args, logits, label_ids, num_labels, domain_logits, domain_label_ids, num_domain_labels,
                                n_gpu=1)
        ...
        if all_domain_logits is None:
            all_domain_logits = domain_logits
        else:
            all_domain_logits = torch.cat((all_domain_logits, domain_logits), 0)
        accum_domain_preds, accum_domain_label_ids = accum_preds_and_label_ids(accum_domain_preds,
                                                                               accum_domain_label_ids, domain_preds,
                                                                               domain_label_ids,
                                                                               is_tok_cls=is_tok_cls)
        eval_task_loss += task_loss.item()
        eval_dom_loss += domain_loss.item()
        eval_loss += loss.item()
        nb_eval_steps += 1

    eval_loss = eval_loss / nb_eval_steps
    eval_task_loss = eval_task_loss / nb_eval_steps
    eval_dom_loss = eval_dom_loss / nb_eval_steps
    eval_acc = compute_metrics(accum_preds, accum_label_ids)[args.metric_to_use]
    try:
        eval_dom_acc = compute_metrics(accum_domain_preds, accum_domain_label_ids)['acc']
        # print(f"accum_domain_preds = {accum_domain_preds}")
        # print(f"accum_domain_label_ids = {accum_domain_label_ids}")
    except:
        eval_dom_acc = -1.0
    model.train()
    return eval_acc, eval_loss, eval_dom_acc, eval_task_loss, eval_dom_loss
...
def load_model_and_tokenizer(args, num_labels, num_domains, device, n_gpu):
    model = BertForClassificationDan.from_pretrained('bert-base-uncased',
...
def convert_examples_to_tok_level_features(examples, label_list, domains_list, max_seq_length, tokenizer, logger,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_ids, input_mask, segment_ids, output_ids = token_classification_batch_encode_plus(example.text_a,
                                                                                                ...
                                                                                                max_seq_length)
        sanity_checks([input_ids], [input_mask], [segment_ids], max_seq_length)
        domain_labels = [example.domain_label if msk == 1 else -100 for msk in input_mask]
        if ex_index < 1 and do_print_data_sample:
...
def main():
    ...
    parser.add_argument("--cnn_window_size", type=int, default=9, help="CNN 1D-Conv window size")
    parser.add_argument("--cnn_out_channels", type=int, default=16, help="CNN 1D-Conv out channels")
    ...
    model, tokenizer = load_model_and_tokenizer(args, num_labels, len(domain_label_dict), device, n_gpu)
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss, task_loss, domain_loss, preds, label_ids, domain_preds, domain_label_ids = \
                    calc_loss_and_preds(args, logits, label_ids, num_labels, domain_logits, domain_label_ids,
                                        len(domain_label_dict), n_gpu)
            ...
            print(f"Domain dev accuracy: {eval_results[2]}, Domain dev loss: {eval_results[4]}")
